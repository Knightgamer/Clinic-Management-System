...
app.get("/", (req, res) => {
  ...
  } else res.render("login", { root: __dirname }); // Changed this to index cause thats the first page
});
  ...
  for (let i = 0; i < Users.length; i++) {
    if (
      ...
      if (Users[i].role === "Receptionist") {
        ...
        console.log("Todays Date", todaysDate);
        console.log("Patient information", Patients);
        return res.render("doctor", { user: Users[i], todaysDate, Patients });
      }
      // Break out of the loop since user is found and authenticated
      break;
    }
  }

  // If no user was authenticated, render the login page with an error message
  if (!isAuthenticated) {
...
app.post("/patients/:patientid/newrecord", urlEncoder, (req, res) => {
  ...
  if (
    typeof patient.medicalRecords !== "undefined" &&
    Array.isArray(patient.medicalRecords)
  ) {
...
app.delete("/patients/:patientID/:id", (req, res) => {
  ...
  if (recordIndex !== -1) {
    console.log(
      "Patients[patientIndex].medicalRecords: ",
      Patients[patientIndex].medicalRecords
    );
...
app.post("/patients/:patientID/:recordID", (req, res) => {
  ...
  if (recordIndex !== -1) {
    ...
    Patients[patientIndex].medicalRecords[recordIndex].sessionnotes =
      updatedNotes;
    Patients[patientIndex].medicalRecords[recordIndex].prescription =
      updatedPrescription;
    console.log(Patients);
    writeFileSync(patientJSON, JSON.stringify(Patients, null, 2));
...
app.post("/receptionist/patients/:patientID", (req, res) => {
  ...
  let { updatedName, updatedAge, updatedSex, updatedBloodType, updatedPhone } =
    req.body;
  console.log(
    "Updated Information",
    updatedName,
  ...
  if (patientIndex !== -1) {
    ...
    Patients[patientIndex].patientAge = updatedAge;
    Patients[patientIndex].sex = updatedSex;
    console.log(Patients);
    writeFileSync(patientJSON, JSON.stringify(Patients, null, 2));
    console.log("Record Updated");
